...
export default class SetFarewell extends Command {
    constructor(client) {
        super(client, {
            name: 'setfarewell',
            description: {
                content: 'Configure the farewell message and channel',
                usage: '<#channel | disable> [message]',
                examples: ['setfarewell #goodbye Goodbye {user}!', 'setfarewell disable'],
            },
            aliases: ['farewell', 'goodbye'],
            category: 'config',
            cooldown: 5,
            args: true,
            permissions: {
            ...
            options: [
                ...
                {
                    name: "message",
                    description: "The farewell message ({user}, {username}, {server}, {membercount})",
                    type: 3,
                    required: false,
                },
                {
                    name: "action",
                    description: "Disable farewell messages",
                    type: 3,
                    required: false,
                    choices: [
                        { name: 'Disable Farewell', value: 'disable' },
                    ],
                },
            ],
        });
    ...
    async run(ctx, args) {
        ...
        if (args[0]?.toLowerCase() === 'disable') {
            ...
            return ctx.sendTypedMessage({
                embed: this.client.embed()
                    ...
                    .setDescription(`${StatusEmojis.success} Farewell messages have been **disabled**.`),
                message: `${StatusEmojis.success} Farewell messages have been **disabled**.`,
            });
        }
        ...
        const farewellMsg = ctx.isInteraction
            ? ctx.interaction.options.getString('message')
            : args.slice(1).join(' ') || null;

        data.farewell.enabled = true;
        data.farewell.channel = channel.id;
        if (farewellMsg) data.farewell.message = farewellMsg;
        await data.save();
        this.client.guildSettings.set(ctx.guild.id, data);

        const displayMsg = data.farewell.message || 'Goodbye {user}!';

        // ─── Embed ───
        const embed = this.client.embed()
            .setColor(this.client.color.success)
            .setTitle(`${StatusEmojis.success} Farewell Configured`)
            .addFields(
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent(`# ${StatusEmojis.success} Farewell Configured`),
            )
        ...
        // ─── Message ───
        const message = [
            `${StatusEmojis.success} **Farewell Configured**`,
